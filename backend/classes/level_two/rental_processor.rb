  ...
  class RentalProcessor
    ...
    def generate_cars(cars_params)
      cars_params.reduce({}) do |cars, car_params|
        cars[car_params['id']] = LevelTwo::Car.new(car_params['id'], car_params['price_per_day'], car_params['price_per_km']) if cars[car_params['id']].nil?
        cars
      end
    end
    ...
    def generate_rentals(rentals_params)
      rentals_params.reduce({}) do |rentals, rental_params|
        ...
        if !car.nil? && rentals[rental_params['id']].nil?
          rentals[rental_params['id']] = LevelTwo::Rental.new(rental_params['id'], car, rental_params['start_date'], rental_params['end_date'], rental_params['distance'])
        end
        rentals
      end
    end
